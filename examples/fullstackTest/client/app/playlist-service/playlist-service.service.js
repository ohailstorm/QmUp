...
.factory('playListService', function($http, socket, $rootScope) {
...
$http.get('/api/playlists').success(function(response) {
      			playlist = response;
      			console.log(response);
      			socket.syncUpdates('playlist', playlist);
    			});
	...
	var playListOperations = {
		...
		addTrack: function (newTrack) {
			...
			//if(playlist.length<=0 || playlist.indexOf(newTrack)<=0)
				$http.post('/api/playlists', newTrack );
		...
		removeTrack: function (trackId) {
			$http.delete('/api/playlists/'+trackId).success( function (response) {
				console.log("deleted");
